...
import { loadBundle, saveBundle } from '../src/bundle.js';
...
test('explicit dry-run plans every label, validates, persists and reloads unchanged', async t => {
  const f = await fixture({ 'one.ts': '/** Returns one. */\nexport function one() { return 1; }' }); t.after(f.cleanup);
  const r = await scan({ mode: 'files', files: ['one.ts'], dryRun: true }, { cwd: f.root, config: { storageDir: f.storageDir }, transport: async () => { throw new Error('dry-run called transport'); } });
...
test('deletion-only code changes select attached documentation', async t => {
  const f = await repository({ 'one.ts': '/** Returns zero for negatives. */\nfunction f(x:number) {\n if (x < 0) return 0;\n return x;\n}\n' }); t.after(f.cleanup);
  await writeFile(join(f.root, 'one.ts'), '/** Returns zero for negatives. */\nfunction f(x:number) {\n return x;\n}\n');
  const r = await scan({ mode: 'working', dryRun: true }, { cwd: f.root, persist: false });
  assert.equal(r.bundle.units.length, 1);
});
...
test('branch rename preserves source paths', async t => {
  ...
  await git(f.root, ['add', '.']); await git(f.root, ['commit', '-m', 'rename']);
...
test('provider execution validates all labels, reuses cache, invalidates on code change', async t => {
  ...
  let calls = 0;
  const options = { cwd: f.root, config: { storageDir: f.storageDir }, persist: false, transport: async (request: Parameters<typeof syntheticResponse>[0]) => { calls++; return syntheticResponse(request); } };
  ...
  assert.equal(calls, 1); assert.equal(second.bundle.coverage.cachedPackets, 1);
  await writeFile(join(f.root, 'one.ts'), '/** Returns one. */\nfunction one() { return 2; }');
  const third = await scan({ mode: 'files', files: ['one.ts'] }, options);
  assert.equal(calls, 2); assert.equal(third.bundle.coverage.cachedPackets, 0);
});
test('missing independent answer preserves valid results and is not cached', async t => {
  const f = await fixture({ 'one.ts': '// returns one\nfunction one() {return 1;}' }); t.after(f.cleanup);
  const r = await scan({ mode: 'files', files: ['one.ts'] }, { cwd: f.root, config: { storageDir: f.storageDir }, persist: false, transport: async request => { const response = syntheticResponse(request); delete response.answers.q_0; return response; } });
  assert.equal(r.bundle.run.status, 'partial');
  assert.equal(r.bundle.coverage.labels.ok, 13); assert.equal(r.bundle.coverage.labels.error, 1);
});
test('extra question ID invalidates entire packet', async t => {
  const f = await fixture({ 'one.ts': '// returns one\nfunction one() {return 1;}' }); t.after(f.cleanup);
  const r = await scan({ mode: 'files', files: ['one.ts'] }, { cwd: f.root, config: { storageDir: f.storageDir }, persist: false, transport: async request => { const response = syntheticResponse(request); response.answers.unexpected = { type: 'noul', noul: 1 }; return response; } });
  assert.equal(r.bundle.coverage.labels.ok, 0); assert.equal(r.bundle.coverage.labels.error, 14);
});
test('cancellation retains completed packet and marks remaining work', async t => {
  const f = await fixture({ 'one.ts': '// one\nfunction one(){return 1;}\n// two\nfunction two(){return 2;}' }); t.after(f.cleanup);
  const controller = new AbortController();
  const r = await scan({ mode: 'files', files: ['one.ts'] }, { cwd: f.root, signal: controller.signal, config: { storageDir: f.storageDir, requestConcurrency: 1 }, persist: false, transport: async request => { controller.abort(); return syntheticResponse(request); } });
  assert.equal(r.bundle.run.status, 'cancelled');
  assert.equal(r.bundle.coverage.labels.ok, 14); assert.equal(r.bundle.coverage.labels.cancelled, 14);
});
test('oversized context skips code-relative labels but runs text labels', async t => {
  const f = await fixture({ 'one.ts': '// describes lots of values\nfunction one(){return [' + '1,'.repeat(500) + '];}' }); t.after(f.cleanup);
  const r = await scan({ mode: 'files', files: ['one.ts'], dryRun: true }, { cwd: f.root, config: { maxContextChars: 256 }, persist: false });
  assert.deepEqual(r.bundle.units[0]!.labels.local_consistency, { status: 'not_evaluated', reason: 'context_prerequisite:complete_local' });
  assert.ok(Object.values(r.bundle.executions).some(e => Object.values(e.bindings).some(b => b.labelId === 'writing_clarity')));
});
test('tampering with persisted excerpts or unknown fields is rejected', async t => {
  const f = await fixture({ 'one.ts': '// hi\nfunction one() {}' }); t.after(f.cleanup);
  const r = await scan({ mode: 'files', files: ['one.ts'], dryRun: true }, { cwd: f.root, config: { storageDir: f.storageDir } });
...
test('UTF-8 BOM and CRLF are preserved in captured source and excerpts', async t => {
  const text = '\uFEFF// exact 🦦\r\nfunction one() {}\r\n';
  const f = await fixture({ 'one.ts': text }); t.after(f.cleanup);
...
test('a file edited during capture is rejected instead of mixing snapshots', async t => {
  ...
  const mocked = t.mock.method(fs, 'readFile', async (...args: Parameters<typeof fs.readFile>) => {
    ...
    if (String(args[0]).endsWith('/one.ts') && !mutated) { mutated = true; await writeFile(args[0], '// changed and longer\nfunction one() {}'); }
    return bytes;
  });
...
test('parse errors retain usable text labels and produce a partial bundle', async t => {
  const f = await fixture({ 'one.ts': '// purpose\nfunction one() { return ??? }' }); t.after(f.cleanup);
  const { bundle } = await scan({ mode: 'files', files: ['one.ts'], dryRun: true }, { cwd: f.root, persist: false });
...
test('renaming a function does not invent a removed unchanged comment', async t => {
  ...
  const { bundle } = await scan({ mode: 'working', dryRun: true }, { cwd: f.root, persist: false });
  assert.equal(bundle.removed.length, 0); assert.equal(bundle.units.length, 1);
});
