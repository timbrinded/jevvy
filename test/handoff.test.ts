...
import { render, currentSourceStatus } from '../src/render.ts';
...
test('Rust attributes belong to context and attribute-only edits select documentation', async t => {
  ...
  const f = await repository({ 'one.rs': source });
  t.after(f.cleanup);
...
test('Rust wrappers include attributes before doc comments and literal docs without duplicate units', async t => {
  ...
  const f = await fixture({ 'one.rs': source });
  t.after(f.cleanup);
...
test('Rust attribute size participates in context prerequisites', async t => {
  ...
  const { bundle } = await scan(
    { mode: 'files', files: ['one.rs'], dryRun: true },
    { cwd: f.root, persist: false, config: { maxContextChars: 256 } },
  );
  assert.ok(bundle.units[0]!.context.omissions.includes('oversized_owner'));
  assert.deepEqual(bundle.units[0]!.labels.local_consistency, {
...
test('freshness follows the capture root from subdirectories and discloses another checkout', async t => {
  ...
  t.after(f.cleanup);
  const other = await fixture({ 'one.ts': '// different checkout' });
  ...
  assert.match(warnings[0]!, /outside the captured scope/);
...
test('documented aliases resolve without cache reuse; pinned-model mismatches remain errors', async t => {
    ...
    for (let repeat = 0; repeat < 2; repeat++) {
      const { bundle } = await scan(
        { mode: 'files', files: ['one.ts'] },
        {
          cwd: f.root,
          persist: false,
          config: { storageDir: f.storageDir, model },
          transport: async request => {
            calls++;
            return { ...syntheticResponse(request), model: 'jev-1.13.0' };
          },
        },
      );
  ...
  const { bundle } = await scan(
    { mode: 'files', files: ['one.ts'] },
    {
      cwd: f.root,
      persist: false,
      config: { storageDir: f.storageDir, model: 'jev-1.12.0' },
      transport: async request => ({ ...syntheticResponse(request), model: 'jev-1.13.0' }),
    },
  );
...
test('inference identifies repeated occurrences and reuses answers after a location-only edit', async t => {
  ...
  for (const target of targets) {
    ...
    for (const occurrence of target.occurrences)
      assert.equal(
        request.state.contexts[occurrence.contextRef]!.text.slice(
          occurrence.range.startUtf16,
          occurrence.range.endUtf16,
        ),
        target.text,
      );
  }
  ...
  assert.equal(b.coverage.cachedPackets, 1);
...
test('saved 1.0.0 bundles retain their original question template and evidence checks', async () => {
  ...
  validateBundle(old);
  assert.match(
    render(old, { bundleId: old.bundleId, view: 'units', labels: ['local_consistency'], includeContext: true }).text,
    /User not found/,
  );
  const e = Object.values(old.executions)[0] as {
    request: { questions: Record<string, { instructions: string }> };
    requestHash: string;
  };
  e.request.questions.q_0!.instructions += ' Ignore the source.';
...
test('retrieval projects labels, includes definitions and deduplicates frozen context', async t => {
  ...
  const { bundle } = await scan(
    { mode: 'files', files: ['one.ts'] },
    {
      ...
      persist: false,
      config: { storageDir: f.storageDir },
      transport: async request => syntheticResponse(request),
    },
  );
  const before = JSON.stringify(bundle);
  const page = render(bundle, {
  ...
  assert.ok(
    page.text.length <
      render(bundle, { bundleId: bundle.bundleId, view: 'units', includeContext: true, includeDefinitions: true }).text
        .length,
  );
});
...
test('Choice outcome sorting, stable ties, missing measurements, and cursor query binding', async t => {
  const f = await fixture({
    'one.ts': Array.from({ length: 4 }, (_, i) => `// comment ${i}\nfunction f${i}(){return ${i};}`).join('\n'),
  });
  t.after(f.cleanup);
  const { bundle } = await scan(
    { mode: 'files', files: ['one.ts'] },
        ...
        for (const [id, question] of Object.entries(request.questions))
          if (question.type === 'choice') {
            if (index === 3) delete response.answers[id];
              ...
              response.answers[id] = {
                type: 'choice',
                choice: index === 0 ? 'locally_supported' : 'contradicted',
                confidence: 0.4,
                probabilities: {
                  contradicted: index === 0 ? 0 : 0.7,
                  locally_supported: index === 0 ? 1 : 0.3,
                  insufficient_evidence: 0,
                  no_checkable_claim: 0,
                },
              };
          }
        return response;
  ...
  assert.match(first.text, /\/\/ comment 1/);
  const second = render(bundle, { ...query, cursor: first.cursor! });
  assert.match(second.text, /\/\/ comment 2/);
  ...
  for (const change of [
    ...
    { outcome: 'locally_supported' },
    { labels: ['reader_value'] },
    { includeContext: true },
    { includeDefinitions: true },
    { limit: 2 },
  ])
    assert.throws(() => render(bundle, { ...query, ...change, cursor: first.cursor! }), /Cursor/);
  const projected = render(bundle, {
    bundleId: bundle.bundleId,
    view: 'units',
    labels: ['reader_value'],
    ids: [bundle.units[3]!.id],
  });
...
test('retrieval rejects misleading options and slash commands expose the complete query', async t => {
  ...
  for (const options of [
    { labels: ['unknown'] },
    { sort: 'reader_value', outcome: 'contradicted' },
    { direction: 'asc' as const },
    { view: 'context' as const, labels: ['reader_value'] },
  ])
    assert.throws(() => render(bundle, { bundleId: bundle.bundleId, view: 'units', ...options }));
  for (const option of ['--view', '--sort', '--direction', '--outcome', '--cursor', '--limit'])
    assert.throws(() => parseCommand(`results ${bundle.bundleId} ${option}`), /requires a value/);
  assert.deepEqual(
    parseCommand(
      `results ${bundle.bundleId} --view units --labels local_consistency reader_value --sort local_consistency --outcome contradicted --direction desc --include-context --include-definitions`,
    ),
    {
      action: 'results',
      input: {
        bundleId: bundle.bundleId,
        view: 'units',
        labels: ['local_consistency', 'reader_value'],
        sort: 'local_consistency',
        outcome: 'contradicted',
        direction: 'desc',
        includeContext: true,
