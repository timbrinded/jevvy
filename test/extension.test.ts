...
test('slash handler yields, cancellation aborts transport, and shutdown retains the cancelled bundle', async t => {
  ...
  const previousConfig = process.env.JEVVY_CONFIG, previousKey = process.env.TYPESAFE_API_KEY;
  process.env.JEVVY_CONFIG = JSON.stringify({ storageDir: f.storageDir });
  process.env.TYPESAFE_API_KEY = 'test-only';
  t.after(() => {
    if (previousConfig === undefined) delete process.env.JEVVY_CONFIG; else process.env.JEVVY_CONFIG = previousConfig;
  ...
  const messages: unknown[] = [], notices: string[] = [];
  extension({
    registerTool() {},
  ...
  const ctx = { cwd: f.root, hasUI: true, ui: { notify(message: string) { notices.push(message); }, setWidget() {} } } as unknown as Parameters<typeof command.handler>[1];
  const timeout = new Promise<never>((_resolve, reject) => { const id = setTimeout(() => reject(new Error('command did not yield or transport did not start')), 5000); t.after(() => clearTimeout(id)); });
