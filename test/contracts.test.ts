...
import { BundleSchema, type Question } from '../src/contracts.ts';
import { answerError, validateBundle } from '../src/validate.ts';
...
test('wrong types, missing levels, extra fields, non-finite and coerced values are rejected', () => {
  for (const bad of [
    { ...answer, score: '2.97' },
    { ...answer, surprise: true },
    { ...answer, score: Infinity },
    { ...answer, confidence: NaN },
    { ...answer, probabilities: { 0: 0, 3: 1 } },
    { ...answer, legend: { ...answer.legend, 3: 'wrong' } },
  ])
    assert.ok(answerError(bad, question));
});
test('Choice supports ties but rejects non-maximal or invented choices', () => {
  ...
  assert.equal(
    answerError({ type: 'choice', choice: 'b', confidence: 0, probabilities: { a: 0.5, b: 0.5 } }, q),
    undefined,
  );
...
test('semantic checks reject valid-shaped misrouting, wrong coverage and evidence substitution', async t => {
  ...
  const mutate = (fn: (bundle: typeof r.bundle) => void) => {
    const b = structuredClone(r.bundle);
    fn(b);
    assert.throws(() => validateBundle(b), /invariant/);
  };
  mutate(b => {
    b.coverage.labels.ok--;
  });
  mutate(b => {
    Object.values(b.executions)[0]!.bindings.q_0!.labelId = 'local_consistency';
  });
  mutate(b => {
  ...
  mutate(b => {
    delete b.units[0]!.labels.writing_clarity;
  });
  mutate(b => {
    b.units[0]!.context.refs.push('missing');
  });
  mutate(b => {
    b.units[0]!.context.status = 'partial';
  });
});
