...
test('conflicting process-wide grammar registration fails explicitly', () => {
  ...
  assert.match(result.stdout, /not supported|conflict|unavailable/);
});
test('SDK retries once under its own policy, engine does not multiply retries', async t => {
  ...
  const client = new TypeSafeClient({
    ...
    fetch: async (_url, init) => {
      ...
      return new Response(JSON.stringify(syntheticResponse(request)), {
  ...
  const r = await scan(
    { mode: 'files', files: ['one.ts'] },
    {
      ...
      transport: (request, signal) =>
        client.systemOne({ ...request, questions: request.questions as Record<string, Question> }, { signal }),
    },
  );
...
test('overall deadline interrupts an in-flight request and keeps a valid cancelled bundle', async t => {
  ...
  const r = await scan(
    { mode: 'files', files: ['one.ts'] },
    {
      cwd: f.root,
      persist: false,
      config: { storageDir: f.storageDir, runTimeoutMs: 100 },
...
test('request concurrency is bounded across unrelated functions', async t => {
  const f = await fixture({
    'one.ts': Array.from({ length: 6 }, (_, i) => `// ${i}\nfunction f${i}() {return ${i};}`).join('\n'),
  });
  t.after(f.cleanup);
  let active = 0,
    maximum = 0;
  const r = await scan(
    { mode: 'files', files: ['one.ts'] },
    {
      cwd: f.root,
      persist: false,
      config: { storageDir: f.storageDir, requestConcurrency: 2 },
  ...
  assert.equal(r.bundle.coverage.labels.ok, 84);
});
...
test('small request budgets split packets without losing or duplicating labels', async t => {
  ...
  const { bundle } = await scan(
    ...
    { cwd: f.root, persist: false, config: { maxRequestBytes: 3000 } },
  );
