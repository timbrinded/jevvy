...
export const clean = value => String(value).replace(/\x1b\][^\x07]*(?:\x07|\x1b\\)/g, '').replace(/\x1b\[[0-?]*[ -/]*[@-~]/g, '').replace(/[\x00-\x1f\x7f-\x9f]/g, ' ');
const frames = ['⠋', '⠙', '⠹', '⠸', '⠼', '⠴', '⠦', '⠧', '⠇', '⠏'];
...
export function panel(state, width, theme, now = Date.now()) {
  ...
  const lines = width < 60
    ? [heading, meter + fg('muted', detail), fg('dim', `${state.mode === 'live' ? '' : 'Injected ' + state.mode + ' · '}/ui-lab cancel`)]
    : [heading, fg('muted', clean(state.path)), meter + fg('muted', detail)];
  if (width >= 60) lines.push(fg('dim', `${state.mode === 'live' ? 'Live API' : 'Injected ' + state.mode} · /ui-lab cancel`));
