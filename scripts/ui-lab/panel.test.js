...
const state = {
  phase: 'Analysing comments',
  path: '深い/👩🏽‍💻/é/' + 'long path '.repeat(80),
  started: 0,
  mode: 'live',
  total: 31,
  done: 9,
...
test('unknown totals never become fabricated percentages, and static mode stops the spinner', () => {
  const a = panel({ ...state, total: 0, static: true }, 80, theme, 1600)
    .map(clean)
    .join('\n');
  const b = panel({ ...state, total: 0, static: true }, 80, theme, 1760)
    .map(clean)
    .join('\n');
  assert.match(a, /3 requests in flight/);
  assert.doesNotMatch(a, /%|31/);
...
function harness(hasUI = true) {
  ...
  return {
    command: args => commands.get('ui-lab').handler(args, ctx),
    events,
    renderers,
    messages,
    widgets,
...
test('headless error path retains model evidence and compact/expanded results remain distinct', async () => {
  ...
  assert.match(m.details.summary, /112 errors/);
