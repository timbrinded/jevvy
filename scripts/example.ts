...
import { updateCoverage } from '../src/bundle.ts';
import { validateBundle } from '../src/validate.ts';
import { scanReport } from '../src/render.ts';
import type { Answer } from '../src/contracts.ts';
...
try {
  ...
  bundle.run.mode = 'example';
  bundle.run.scope.root = '/example';
  ...
  for (const [packetId, execution] of Object.entries(bundle.executions)) {
    ...
    for (const [questionId, binding] of Object.entries(execution.bindings)) {
      ...
      if (question.type === 'noul')
        answer = {
          type: 'noul',
          noul: binding.labelId === 'explains_rationale' ? (unit.structure.syntax === 'line' ? 0.98 : 0.03) : 0.5,
        };
  ...
  if (bundle.units.length !== 2 || Object.keys(bundle.contexts).length !== 1 || bundle.coverage.labels.ok !== 28)
    throw new Error('Example must demonstrate two comments sharing one context and all 28 labels');
  await mkdir('examples', { recursive: true });
  await writeFile('examples/jevvy-results.example.json', JSON.stringify(bundle, null, 2) + '\n');
  await writeFile(
    'examples/jevvy-results.example.md',
