...
const CacheSchema = Type.Object({
  key: Type.String(), bundleId: Type.String(),
  response: Type.Object({ model: Type.String(), usage: UsageSchema, answers: Type.Record(Type.String(), AnswerSchema) }, { additionalProperties: false }),
...
export async function storeCache(config: Config, bundle: Bundle, execution: Execution, response: ReturnType<typeof validateResponse>): Promise<void> {
  ...
  try { await writeFile(join(config.storageDir, 'cache', `${key}.json`), JSON.stringify(value), { mode: 0o600, flag: 'wx' }); }
