...
export function planRequests(bundle: Bundle, config: Config): void {
  ...
  for (const units of groups.values()) {
    let request: CurrentRequest = {
      model: config.model,
      state: { formatVersion: '2', instruction, contexts: {}, comments: {} },
      questions: {},
    };
    ...
    const add = (unit: Unit, labelId: string) => {
      ...
      request.questions[qid] = questionFor(
        definitions[labelId]!,
        targetId,
        request.state.comments[targetId]!.contextRefs,
      );
    ...
    for (const unit of units)
      for (const labelId of Object.keys(definitions)) {
        ...
        if (Buffer.byteLength(JSON.stringify(request)) > config.maxRequestBytes) {
          ...
          targetMap = previous.targetMap;
          contextMap = previous.contextMap;
          flush();
          add(unit, labelId);
          if (Buffer.byteLength(JSON.stringify(request)) > config.maxRequestBytes) {
            request = {
