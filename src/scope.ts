...
import { languageFor } from './ast.js';
import type { Bundle, Language, Range, ScanInput } from './contracts.js';

const exec = promisify(execFile);
export async function git(root: string, args: string[]): Promise<string> {
  return (await exec('git', ['--no-pager', ...args], { cwd: root, encoding: 'utf8', maxBuffer: 64 * 1024 * 1024 })).stdout;
}
export interface Change { oldStart: number; oldCount: number; newStart: number; newCount: number }
export interface CapturedFile { path: string; oldPath: string; language: Language; before: string | null; content: string | null; renamed: boolean; changes: Change[] }
export interface Capture { scope: Bundle['run']['scope']; files: CapturedFile[]; outcomes: Bundle['coverage']['files']; diagnostics: string[] }

async function captureText(root: string, path: string): Promise<string | null> {
  const absolute = resolve(root, path);
  const rel = relative(root, absolute);
  if (rel === '..' || rel.startsWith(`..${sep}`) || isAbsolute(rel) || rel.split(sep).includes('.git')) throw new Error('Selected path is outside source scope');
  let first;
  try { first = await lstat(absolute); } catch (e) { if ((e as NodeJS.ErrnoException).code === 'ENOENT') return null; throw e; }
  ...
  if (actualRel === '..' || actualRel.startsWith(`..${sep}`) || isAbsolute(actualRel)) throw new Error('Symlinked parent escapes source root');
  const bytes = await readFile(absolute);
  const after = await lstat(absolute);
  if (first.ino !== after.ino || first.size !== after.size || first.mtimeMs !== after.mtimeMs || first.ctimeMs !== after.ctimeMs) throw new Error('File changed during capture; rerun to capture a consistent version');
  return new TextDecoder('utf-8', { fatal: true, ignoreBOM: true }).decode(bytes);
}
async function blob(root: string, revision: string, path: string): Promise<string | null> {
  try {
    const result = await exec('git', ['--no-pager', 'show', `${revision}:${path}`], { cwd: root, encoding: 'buffer', maxBuffer: 64 * 1024 * 1024 });
...
async function diff(before: string, content: string): Promise<Change[]> {
  ...
  try {
    ...
    let patch: string;
    try { patch = await git(directory, ['diff', '--no-index', '--no-ext-diff', '--no-textconv', '--diff-algorithm=myers', '--text', '--unified=0', '--', 'before', 'after']); }
    catch (error) {
      const e = error as Error & { code?: number; stdout?: string };
      if (e.code !== 1 || typeof e.stdout !== 'string') throw error;
      patch = e.stdout;
    }
    return [...patch.matchAll(/^@@ -(\d+)(?:,(\d+))? \+(\d+)(?:,(\d+))? @@/gm)].map(m => ({ oldStart: Number(m[1]), oldCount: Number(m[2] ?? 1), newStart: Number(m[3]), newCount: Number(m[4] ?? 1) }));
...
export function affected(range: Range, changes: Change[], side: 'old' | 'new'): boolean {
  return changes.some(c => {
    ...
    return count === 0 ? start >= range.startLine - 1 && start <= range.endLine : start <= range.endLine && start + count - 1 >= range.startLine;
  });
}
export async function captureScope(input: ScanInput, cwd: string, signal?: AbortSignal): Promise<Capture> {
  ...
  const resolveRef = async (ref: string) => (await git(root, ['rev-parse', '--verify', '--end-of-options', `${ref}^{commit}`])).trim();
  const head = input.mode === 'files' ? null : await resolveRef(input.mode === 'branch' ? input.head ?? 'HEAD' : 'HEAD');
  ...
  if (input.mode === 'files') {
    for (const path of input.files!) candidates.push({ path: relative(root, resolve(root, path)).split(sep).join('/'), oldPath: path, renamed: false });
  } else if (input.mode === 'branch') {
    const fields = (await git(root, ['diff', '--no-ext-diff', '--no-textconv', '--name-status', '-z', '-M', mergeBase!, head!, '--'])).split('\0');
    ...
    const paths = new Set((await git(root, ['ls-files', '-z', '--cached', '--others', '--exclude-standard'])).split('\0').filter(Boolean));
    for (const path of (await git(root, ['ls-tree', '-r', '--name-only', '-z', head!])).split('\0').filter(Boolean)) paths.add(path);
    ...
    const fields = (await git(root, ['diff', '--no-ext-diff', '--no-textconv', '--name-status', '-z', '-M', head!, '--'])).split('\0');
    const renames = new Map<string, string>();
    for (let i = 0; i < fields.length && fields[i];) {
      const status = fields[i++]!, first = fields[i++]!;
      if (status.startsWith('R')) { const dest = fields[i++]!; renames.set(dest, first); paths.delete(first); }
    }
  ...
  for (const candidate of [...unique.values()].sort((a, b) => a.path.localeCompare(b.path, 'en'))) {
    ...
    if (signal?.aborted) { result.outcomes.push({ path, status: 'cancelled', reason: 'Capture cancelled' }); continue; }
    const language = languageFor(path) ?? languageFor(oldPath);
    if (!language) { result.outcomes.push({ path, status: 'unsupported', reason: 'No comments adapter for this extension' }); continue; }
    try {
