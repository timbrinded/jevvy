...
export function initializeParsers(): void {
  ...
  if (!['darwin-arm64', 'darwin-x64', 'linux-arm64', 'linux-x64'].includes(platform)) throw new Error(`Unsupported native parser platform: ${platform}`);
  registerDynamicLanguage({ python, rust, solidity: {
    libraryPath: fileURLToPath(new URL(`../native/solidity/${platform}/tree-sitter-solidity.node`, import.meta.url)),
    extensions: ['sol'], languageSymbol: 'tree_sitter_solidity',
  } });
  // Another extension may have consumed the process-wide one-shot registry.
  for (const [lang, example] of [['python', '# probe\nx = 1'], ['rust', '// probe\nfn f() {}'], ['solidity', '// probe\ncontract C {}']] as const) {
    const root = parse(lang, example).root();
    if (root.findAll({ rule: { kind: 'ERROR' } }).length || !walk(root).some(n => String(n.kind()).includes('comment'))) throw new Error(`Native ${lang} grammar registration unavailable or conflicting`);
  }
  initialized.add(parse);
}
export function languageFor(path: string): Language | undefined {
  return ({ '.ts': 'typescript', '.mts': 'typescript', '.cts': 'typescript', '.tsx': 'tsx', '.rs': 'rust', '.py': 'python', '.sol': 'solidity' } as Record<string, Language>)[extname(path).toLowerCase()];
}
...
export function rangeFor(content: string, start: number, end: number): Range {
  return { startUtf16: start, endUtf16: end, startLine: content.slice(0, start).split('\n').length, endLine: content.slice(0, end).split('\n').length };
}
export function overlaps(a: Range, b: Range): boolean { return a.startUtf16 < b.endUtf16 && b.startUtf16 < a.endUtf16; }
