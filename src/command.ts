...
export function parseCommand(text: string): { action: 'comments'; input: ScanInput } | { action: 'results'; input: ResultsInput } | { action: 'inspect'; bundleId: string } | { action: 'cancel' } {
  ...
  if (action === 'results') {
    ...
    const next = (flag: string) => { const value = args.shift(); if (!value || value.startsWith('--')) throw new Error(`${flag} requires a value`); return value; };
    while (args.length) {
      ...
      else if (flag === '--labels') { input.labels = []; while (args.length && !args[0]!.startsWith('--')) input.labels.push(args.shift()!); }
  ...
  if (action !== 'comments') throw new Error('Usage: /jevvy comments --files <paths> | --working | --base <ref> [--head <ref>] [--dry-run]; /jevvy results <bundle-id>; /jevvy inspect <bundle-id>; /jevvy cancel');
