...
import type { Bundle, Config } from './contracts.ts';
...
export function updateCoverage(bundle: Bundle): void {
  ...
  bundle.coverage.units = {
    selected: bundle.units.length,
    excluded: bundle.excluded.length,
    removed: bundle.removed.length,
  };
...
async function prune(config: Config): Promise<void> {
  ...
  for (const directory of ['bundles', 'cache']) {
    let names: string[];
    try {
      names = await readdir(join(config.storageDir, directory));
    } catch (error) {
