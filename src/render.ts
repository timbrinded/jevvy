...
function value(unit: Unit, label: string, outcome?: string): number | undefined {
  ...
  return result.answer.type === 'score' ? result.answer.score : result.answer.type === 'noul' ? result.answer.noul : result.answer.probabilities[outcome ?? result.answer.choice]!;
}
...
export interface Page { bundleId: string; view: string; order: string; selection: string[] | null; returned: number; total: number; cursor: string | null; labels: string[] | null; includeContext: boolean; includeDefinitions: boolean; text: string }
export function render(bundle: Bundle, input: ResultsInput, options: { compact?: boolean; includeHeader?: boolean } = {}): Page {
  ...
  const view = input.view ?? 'overview', limit = input.limit ?? 5;
  const direction = input.direction ?? 'desc';
  const includeContext = input.includeContext ?? false, includeDefinitions = input.includeDefinitions ?? false;
  if (input.sort && !bundle.definitions[input.sort]) throw new Error('Unknown label sort');
  if (view !== 'units' && (input.sort || input.direction || input.outcome || includeContext)) throw new Error('Sorting and combined evidence are only available for units');
  ...
  if (input.outcome) {
    ...
    if (definition.primitive !== 'choice' || !Object.hasOwn(definition.criteria, input.outcome)) throw new Error('Unknown Choice outcome for sort');
  }
  if (view === 'context' && (input.labels || includeDefinitions)) throw new Error('Context view has no labels or definitions');
  ...
  const order = input.sort ? `${input.sort}${bundle.definitions[input.sort]!.primitive === 'choice' ? `[${input.outcome ?? 'winner_probability'}]` : ''}:${direction}` : 'source';
  const selection = input.ids ? [...new Set(input.ids)].sort() : null;
  const selectionHash = hash({ bundleId: bundle.bundleId, view, order, selection, labels: selectedLabels, includeContext, includeDefinitions, limit });
  let offset = 0;
  if (input.cursor) {
    try {
    ...
    } catch { throw new Error('Cursor does not match this query. Repeat the same bundleId, view, ids, labels, sort, direction, outcome, includeContext, includeDefinitions and limit; omit cursor to start a different query.'); }
  }
  const header = [
  ...
  if (view === 'overview' || includeDefinitions) {
    ...
    for (const [id, d] of Object.entries(bundle.definitions).filter(([id]) => !selectedLabels || selectedLabels.includes(id))) blocks.push(`${id} (${d.primitive}; requires ${d.requires}): ${d.question}${Array.isArray(d.criteria) ? '\n' + d.criteria.map((v, i) => `  ${i}: ${v}`).join('\n') : '\n  ' + JSON.stringify(d.criteria)}`);
  ...
  if (view !== 'overview') {
    ...
    if (view === 'units' && input.sort) {
      ...
      ids.sort((a, b) => {
        const av = value(units.get(a)!, input.sort!, input.outcome), bv = value(units.get(b)!, input.sort!, input.outcome);
        // Missing measurements always follow measured ones, in either direction.
        if (av === undefined || bv === undefined) return av === bv ? sourceOrder.get(a)! - sourceOrder.get(b)! : av === undefined ? 1 : -1;
    ...
    for (const id of page) {
      if (view === 'context') {
        ...
        const labels = Object.entries(u.labels).filter(([id]) => !selectedLabels || selectedLabels.includes(id)).map(([id, label]) => ({ group: bundle.definitions[id]!.group, text: `${id}: ${label.status === 'ok' ? summary(label.answer, !options.compact) : `${label.status} (${label.reason})`}` }));
        const labelText = options.compact ? [labels.filter(l => l.group === 'purpose').map(l => l.text).join('; '), ...labels.filter(l => l.group !== 'purpose').map(l => l.text)].join('\n') : labels.map(l => l.text).join('\n');
        const omittedStatuses = Object.entries(u.labels).filter(([id, label]) => selectedLabels && !selectedLabels.includes(id) && label.status !== 'ok');
        const coverageNotes = omittedStatuses.map(([id, label]) => `${id}: ${label.status}${'reason' in label ? ` (${label.reason})` : ''}`);
        for (const ref of u.context.refs) if (includeContext) includedContexts.add(ref);
        const text = options.compact && u.text.length > 500 ? u.text.slice(0, 500) + '\n[Comment excerpt; retrieve units view for full text.]' : u.text;
  ...
  header.push(`View=${view}; order=${order}; selection=${selection ? selection.join(',') : 'all'}; labels=${selectedLabels?.join(',') ?? 'all'}; includeContext=${includeContext}; includeDefinitions=${includeDefinitions}; returned=${returned}; total=${total}; cursor=${next ?? 'none'}`);
    if (next) blocks.push(options.includeHeader === false ? 'More results are available on the next page.' : 'This page is not the complete result. Continue with the cursor above and the same query options, including limit. Omit cursor when changing the query.');
  return { bundleId: bundle.bundleId, view, order, selection, returned, total, cursor: next, labels: selectedLabels, includeContext, includeDefinitions, text: [...(options.includeHeader === false ? [] : [...header, '']), ...blocks].join('\n\n') };
}
...
export async function currentSourceStatus(bundle: Bundle, cwd: string): Promise<string[]> {
  ...
  if (location === '..' || location.startsWith(`..${sep}`) || isAbsolute(location)) {
    results.push(`Current directory is outside the captured scope ${root}; freshness is checked against that captured root, not this checkout`);
  }
