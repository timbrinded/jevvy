...
export const RangeSchema = object({ startUtf16: count, endUtf16: count, startLine: Type.Integer({ minimum: 1 }), endLine: Type.Integer({ minimum: 1 }) });
export const DefinitionSchema = object({
  group: Type.Enum(['purpose', 'quality', 'consistency']),
  primitive: Type.Enum(['noul', 'choice', 'score']), question: id,
...
export const AnswerSchema = Type.Union([
  ...
  object({ type: Type.Literal('score'), score: Type.Number({ minimum: 0, maximum: 9 }), probabilities: dictionary(probability), legend: dictionary(text), confidence: probability }),
]);
...
export const SourceSchema = object({
  path: id, snapshot: Type.Enum(['before', 'captured']), contentHash: id, encoding: Type.Literal('utf-8'),
  language: LanguageSchema, content: text,
});
export const ContextSchema = object({ sourceId: id, range: RangeSchema, role: Type.Enum(['owner', 'header', 'surroundings']), text });
...
export const UnitSchema = object({
  id, sourceId: id, range: RangeSchema, text, structure: StructureSchema,
  change: Type.Enum(['added', 'modified', 'associated_code', 'unchanged']),
  context: object({ status: Type.Enum(['complete_local', 'partial', 'unavailable']), refs: Type.Array(id), omissions: Type.Array(id) }),
...
export const ExecutionSchema = object({
  requestHash: id, request: RequestSchema, bindings: dictionary(BindingSchema),
  origin: Type.Enum(['planned', 'provider', 'cache', 'synthetic']),
  status: Type.Enum(['planned', 'ok', 'partial', 'error', 'cancelled']),
  model: nullable(id), usage: nullable(UsageSchema),
  cacheSource: nullable(id), diagnostics: Type.Array(text),
});
...
export const BundleSchema = Type.Object({
  ...
  run: object({
    mode: Type.Enum(['live', 'dry_run', 'example']), status: Type.Enum(['completed', 'partial', 'cancelled', 'failed']),
    scope: object({ mode: Type.Enum(['files', 'working', 'branch']), root: id, files: Type.Array(id), base: nullable(id), head: nullable(id), mergeBase: nullable(id) }),
    snapshotId: id, requestedModel: id, resolvedModel: nullable(id), startedAt: id, finishedAt: id,
  }),
  definitions: dictionary(DefinitionSchema), sources: dictionary(SourceSchema), contexts: dictionary(ContextSchema),
  units: Type.Array(UnitSchema), excluded: Type.Array(InventorySchema), removed: Type.Array(InventorySchema),
...
export const ScanInputSchema = object({
  mode: StringEnum(['files', 'working', 'branch'] as const), files: Type.Optional(Type.Array(id, { minItems: 1 })),
  base: Type.Optional(id), head: Type.Optional(id), dryRun: Type.Optional(Type.Boolean()),
});
...
export const ConfigSchema = object({
  ...
  maxContextChars: Type.Integer({ minimum: 256 }), maxRequestBytes: Type.Integer({ minimum: 2048 }),
  requestTimeoutMs: Type.Integer({ minimum: 100 }), runTimeoutMs: Type.Integer({ minimum: 100 }),
  maxRetries: Type.Integer({ minimum: 0, maximum: 5 }), retentionDays: Type.Integer({ minimum: 1 }),
  storageDir: id,
});
