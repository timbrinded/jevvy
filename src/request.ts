...
export function inferenceTarget(bundle: Bundle, unit: Unit, contextRefs: string[]): InferenceTarget {
  return {
    text: unit.text,
    language: bundle.sources[unit.sourceId]!.language,
    structure: {
      ...unit.structure,
      owner: unit.structure.owner ? { kind: unit.structure.owner.kind, name: unit.structure.owner.name } : null,
    },
    contextRefs,
    contextStatus: unit.context.status,
