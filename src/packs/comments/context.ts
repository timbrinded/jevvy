...
import type { Bundle, Config, Context, Language, Range, Source, Structure, Unit } from '../../contracts.js';
import { EXTRACTION_VERSION, overlaps, parseSource, rangeFor, rangeOf, walk } from '../../ast.js';
import { identity } from '../../hash.js';
import { rules } from './languages/index.js';

type Comment = { nodes: SgNode[]; range: Range; syntax: Structure['syntax']; style: Structure['documentationStyle']; forcedOwner?: SgNode; excluded?: string };
export interface Extracted { units: Unit[]; contexts: Record<string, Context>; excluded: Bundle['excluded']; errors: Range[] }
...
function form(node: SgNode, language: Language): Pick<Comment, 'syntax' | 'style'> {
  ...
  if (language === 'python' && ['string', 'concatenated_string'].includes(kind(node))) return { syntax: 'string_literal', style: 'python_docstring' };
  if (kind(node) === 'attribute_item' || kind(node) === 'inner_attribute_item') return { syntax: 'doc_attribute', style: 'rustdoc' };
...
function inventory(root: SgNode, source: Source): Comment[] {
  const language = source.language, adapter = rules[language], found: Comment[] = [];
  for (const node of walk(root)) {
    ...
    if (!adapter.comments.has(kind(node))) {
      ...
      const docAttribute = language === 'rust' && ['attribute_item', 'inner_attribute_item'].includes(kind(node)) && /^#!?\[\s*doc\s*=\s*(?:"|r#*")/.test(node.text());
    ...
    if (shape.syntax === 'line') range = rangeFor(source.content, range.startUtf16, range.endUtf16 - (node.text().match(/[\r\n]+$/)?.[0].length ?? 0));
    const text = source.content.slice(range.startUtf16, range.endUtf16);
    const licence = /SPDX-License-Identifier:|@license\b|(?:^|\n)\s*(?:\/\/|#|\/\*+|\*)?\s*Copyright\s+(?:(?:\(c\)|©)\s*)?\d{4}/i.test(text);
    const directive = /^(?:\/\/|#|\/\*)\s*(?:eslint-|prettier-ignore|@ts-(?:ignore|expect-error|nocheck|check)|type:\s*ignore|noqa\b|ruff:|fmt:|istanbul\s+ignore|c8\s+ignore|sourceMappingURL=|region\b|endregion\b)/i.test(text);
    if (licence || directive) {
      ...
      if (separator) {
        ...
        found.push({ nodes: [node], range: rangeFor(source.content, range.startUtf16, proseStart), ...shape, excluded: 'machine_directive' });
    ...
    const sameRustScope = language !== 'rust' || previous?.nodes[0]!.text().startsWith('//!') === node.text().startsWith('//!');
    if (shape.syntax === 'line' && previous?.syntax === 'line' && previous.style === shape.style && sameRustScope && !previous.excluded && previous.nodes.at(-1)?.parent()?.id() === node.parent()?.id() && /^[\t ]*\r?\n[\t ]*$/.test(gap)) {
...
function ownerFor(comment: Comment, source: Source): { owner?: SgNode; attachment: Structure['attachment'] } {
  ...
  if (previous && rangeOf(previous).endLine === comment.range.startLine && /^[\t ]*$/.test(source.content.slice(rangeOf(previous).endUtf16, comment.range.startUtf16))) {
...
export async function extractComments(sourceId: string, source: Source, config: Pick<Config, 'maxContextChars'>): Promise<Extracted> {
  const root = await parseSource(source.language, source.content);
  const nodes = walk(root);
  const errors = nodes.filter(n => kind(n) === 'ERROR' || (n.range().start.index === n.range().end.index && n.id() !== root.id())).map(rangeOf);
  const contexts: Record<string, Context> = {}, units: Unit[] = [], excluded: Bundle['excluded'] = [];
  for (const comment of inventory(root, source)) {
    ...
    if (comment.excluded) { excluded.push({ sourceId, range: comment.range, text, reason: comment.excluded }); continue; }
    const { owner, attachment } = ownerFor(comment, source);
    const ownerRange = owner ? rangeOf(owner) : undefined;
    const refs: string[] = [], omissions: string[] = [];
    ...
    if (owner && ownerRange) {
      ...
      for (const ancestor of ancestors(wrapped)) {
        ...
        if (body && rangeOf(body).startUtf16 > rangeOf(ancestor).startUtf16) {
          ...
          else { status = 'partial'; omissions.push('oversized_enclosing_header'); }
        }
      }
      if (errors.some(e => overlaps(e, range) || (e.startUtf16 === e.endUtf16 && e.startUtf16 >= range.startUtf16 && e.startUtf16 <= range.endUtf16))) { status = 'partial'; omissions.push('parse_error'); }
    } else {
      status = 'partial'; omissions.push('ambiguous_attachment');
